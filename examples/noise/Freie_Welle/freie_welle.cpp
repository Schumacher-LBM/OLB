...
superGeometry.rename(0, 2);          // tmp
superGeometry.rename(2, 1);          // alles Fluid = 1

const T Lx = 2.4;                    // Domänenlänge x (PU-metrisch wie bei dir)
const T Ly = physwidth;
const T Lz = physspan;
const T dx = converter.getPhysDeltaX();

// ---------- Punktquelle (Material 3) im Zentrum ----------
{
Vector<T,3> center{ T(0), T(0), T(0) };          // Domänenzentrum
const T rSrc = 1.5 * dx;                         // ~1–2 Zellen Radius
IndicatorSphere3D<T> src(center, rSrc);
  ...
  // }//prepareGeometry
    ...
    void prepareLattice(UnitConverter<T, DESCRIPTOR> const& converter,
      ...
      T /*dampingDepthPU*/, T /*lengthDomain*/, T /*dampingStrength*/)
          ...
          clout << "\nPrepare Lattice ..." << std::endl;

          const T omega = converter.getLatticeRelaxationFrequency();

          // Bulk + Quelle: BGK
          auto bulk = superGeometry.getMaterialIndicator({1,3});
          sLattice.defineDynamics<BulkDynamics>(bulk);

          // Sponge: spezielle Dämpf-Dynamik
          ...
          // (Optional) einfache lineare Dämpfprofil-Parameter an SpongeDynamics:
          // Falls deine OpenLB-Version Parameter am Sponge erwartet, setze hier z.B.:
          // sLattice.setParameter<descriptors::SPONGE_DAMPING>(T(0.02)); // Beispielwert
          // --> Wenn es in deiner Version anders heißt, bitte den passenden enum verwenden.

          sLattice.initialize();
          clout << "Prepare Lattice ... OK\n";
          }//prepare Lattice
 ...
 int main(int argc, char* argv[])
 {
  ...
  // T vtkanzahl=iTmax;///10.;//*10.;
   ...
   std::array<Vector<T,ndim>,2> measurePhysR = {
     Vector<T,ndim>{0.546, 0.0, 0.0},
     Vector<T,ndim>{0.646, 0.0, 0.0}
   };
   ...
   // VOR der for(iT)-Zeitschleife, nur EINMAL:
...
const T rho_ref = physDensity;
   ...
   for (std::size_t iT=0; iT < iTmax; ++iT) {
     ...
     const T betaMax = 0.08; // ggf. 0.03..0.12
auto sponge = superGeometry.getMaterialIndicator({4});

sLattice.setProcessingContext(ProcessingContext::Simulation);
// --- Fallback-Sponge: Mischung in Material 4 (kompatibel ohne forEachCellIf)
const T beta = betaMax; // in der Schleife

for (int iC = 0; iC < loadBalancer.size(); ++iC) {
  if (!loadBalancer.isLocal(iC)) continue;

  // Lattice-Block + zugehörige Geometrie holen
  auto& blockLat  = sLattice.getBlock(iC);

  // Je nach OpenLB-Version heisst das so oder so:
  // auto& blockGeom = superGeometry.getExtendedBlockGeometry(iC);
  auto& blockGeom = superGeometry.getBlockGeometry(iC);
  ...
  for (int iZ=0; iZ<nz; ++iZ) {
    ...
    // Nur Zellen mit Material==4 (Sponge) dämpfen
    if (blockGeom.getMaterial(iX,iY,iZ) != 4) continue;

    auto  cell = blockLat.get(iX,iY,iZ);
    auto& ns   = cell.template get<names::NavierStokes>();

    auto u     = ns.computeU();
    T    rho   = ns.computeRho();

    const T beta = betaMax;   // optional: positionsabhängige Ramp

    u[0] *= (T(1)-beta);
    u[1] *= (T(1)-beta);
    u[2] *= (T(1)-beta);
    rho   = (T(1)-beta)*rho + beta*rho_ref;

    ns.setRhoU(rho, u);
    cell.recomputePopulationsFromMacroscopic();
  }
}
